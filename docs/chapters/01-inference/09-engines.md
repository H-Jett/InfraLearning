...
> **LLM 推理 = 一次吃满算力的 prefill + 一长串喂不饱算力的 decode（memory-bound）。
> 几乎所有优化，都是在跟 decode 的 memory-bound 特性、以及它带来的 KV cache 显存压力作斗争。**

- **decode 慢**（memory-bound）→ **batching**（摊权重，第 3）、**量化权重**（减字节，第 6）、
  **投机解码**（一次验多个，第 7）；
- **KV cache 吃显存** → **PagedAttention**（消碎片，第 4）、**前缀复用**（共享，第 5）、
  **KV 量化 / GQA**（减字节，第 2/6）；
- **怎么衡量** → **TTFT/TPOT/吞吐/goodput/SLO**（第 8）。

把这张图记住，你就有了看任何推理系统的"透视眼"。
