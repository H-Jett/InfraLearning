...
![KV cache 随序列长度线性增长，约 1 万 token 处就超过模型权重本身（橙色虚线）。](figures/ch2_kv_cache_growth.png)
...
> 📖 **参考答案**（想清楚再看）：[Q1](../../qa/02-kv-cache-qa.md#q1) · [Q2](../../qa/02-kv-cache-qa.md#q2) · [Q3](../../qa/02-kv-cache-qa.md#q3)

## 2.9 延伸阅读

- 第 4 章 PagedAttention：从"显存碎片"角度进一步榨干 KV cache 的空间。
- 第 6 章 量化：把 KV cache 的每元素字节从 2 砍到 1。
- 第五部分「训练工程」第 34 章：本章的显存计算思路会直接迁移到**训练显存预算**
...
注意力头 num_attention_heads = 16
KV 头   num_key_value_heads = 8   ← GQA：比 Q 头少！
每头维度 head_dim            = 128
精度                         = bf16 (2 字节)

================================================================
A) 用公式手算 KV cache 大小（batch=1）
================================================================
每个 token 的 KV cache = 2×28×8×128×2 = 114688 字节 = 112.00 KiB
...
================================================================
B) 真机实测 past_key_values 字节数 vs 公式
================================================================
   seq_len |             实测 |             公式 | 是否一致
--------------------------------------------------------
       128 |      14.00 MiB |      14.00 MiB | ✅
...
================================================================
C) GQA 省了多少？KV cache 何时超过模型权重？
================================================================
最大上下文 seq_len = 40960
KV cache（GQA, 8 KV 头）      = 4.38 GiB
KV cache（若 MHA, 16 KV 头）  = 8.75 GiB   → GQA 省了 2.0x
模型权重（596M 参数, bf16） = 1.11 GiB
