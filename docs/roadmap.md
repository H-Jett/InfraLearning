...
> **总目标**：从算法工程师视角，吃透 LLM 的推理与训练 infra，最终能**优化框架提速**、**优化算子 (kernel)**。
>
> 分五部分，由浅入深、前后依赖。这是一份**活的大纲**：随学习进度和你的兴趣随时调整。
> 状态标记：✅ 已完成 · 🔜 进行中 · ⬜ 待学。
...
**目标**：把训练扩展到多卡多机，理解并行策略与通信瓶颈。

| 章 | 主题 | 一句话学到什么 | 状态 |
|----|------|----------------|------|
| 23 | 数据并行 (DP / DDP) | 梯度同步、AllReduce | ⬜ |
| 24 | ZeRO 与 FSDP | 优化器状态/梯度/参数切分 | ⬜ |
| 25 | 张量并行 (TP) | 把单层矩阵切到多卡 | ⬜ |
| 26 | 流水线并行 (PP) | 按层切分与 bubble 问题 | ⬜ |
| 27 | 序列并行 / 3D 并行组合 | 各并行怎么叠加 | ⬜ |
| 28 | NCCL 与集合通信 | ring/tree AllReduce、拓扑与带宽 | ⬜ |
| 29 | 通信-计算重叠、混合精度、激活重计算 | 训练提速与省显存的常用手段 | ⬜ |

> **学完能**：理解并配置大模型的并行训练策略，判断瓶颈在通信还是计算。

---

## 第五部分 · 综合实战 (Capstone)

**目标**：把前四部分串起来，完成端到端的"定位瓶颈 → 落地提速"。

| 章 | 主题 | 状态 |
|----|------|------|
| 30 | 部署优化版推理服务（量化 + PagedAttention + continuous batching + CUDA Graph） | ⬜ |
| 31 | 训练调优（FSDP + FlashAttention + torch.compile，压吞吐） | ⬜ |
| 32 | 自定义融合算子（Triton）集成进模型并测收益 | ⬜ |

> **学完能**：独立完成一次从瓶颈定位到落地提速的端到端优化。
...
```
第一部分(推理) ──► 第二部分(算子) ──► 第三部分(框架) ──► 第五部分(实战)
      │                                                        ▲
      └──────────────► 第四部分(分布式训练) ────────────────────┘
```

- **默认路线**：一 → 二 → 三 → 四 → 五。先把推理这条主线走通（有直观反馈），
  再深入算子和框架（你的两个核心目标），最后补分布式训练，用实战收口。
- **可调整**：如果某阶段更想先学训练，第四部分可以提前到第一部分之后；
  第二、三部分互为支撑，也可交叉进行。

## 调整记录

- 2026-07-16：初版路线图。教学法固定为「概念 → 思考题 → 真机代码练习」，内容力求正确。
